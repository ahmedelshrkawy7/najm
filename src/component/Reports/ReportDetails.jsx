...
// }, [userName,email,password]); 
  ...
  return (
    <form onSubmit={handleSubmit((data)=>console.log(data))}>
       <ReportsHeader title={"تفاصيل البلاغ"} subTitle={"يُرجي ملئ الحقول التالية"}/>
        <div className='px-8 pt-4 pb-8  space-y-6'>
          {/* <input {...register("email")} type='text' placeholder='email'/> */}
          <Textarea errors={errors} control={control}  />
          <Controller control={control} name={"InputControl"} rules={
        {required:"هذا الحقل مطلوب",message:"هذا الحقل مطلوب"}
        }
        render={({field,fieldState})=>(
          <Location name={field.name} field={field}  errors={errors} control={control} src={arrowDown} inpTitle={"هل انت على علم باسماء المشتبه بهم؟"} inputPlaceholder={"نعم/لا"}/>
        )}/>
          <Listinput icon={<PlusOutlined/>}/>
          <div className='flex items-center gap-6 flex-wrap pb-4'>
          <Datepicker />
          <Controller control={control} name='locationInputControl' rules={
        {required:"هذا الحق مطلوب",message:"هذا الحقل مطلوب"}
        }  render={({field,fieldState})=>(
          <Location name={field.name} field={field}  errors={errors} control={control} width={24}  src={location} inpTitle={"مكان حدوث المخالفة"} inputPlaceholder={"شارع"}/>
        )}/>         
